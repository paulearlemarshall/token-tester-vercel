...
import type { AppConfig, FileItem, PromptEntry, ProviderConfig, TabId, TestRun } from './types'
...
interface AppState {
  ...
  loadPricing: (data: Record<string, { input: number; output: number }>) => void
...
export const useStore = create<AppState>((set, get) => ({
  ...
  loadPricing: (data) => set(() => {
    const next = migratePricingKeys(data)
    saveJSON('token-tester-model-pricing', next)
    return { modelPricing: next }
  }),
  setModelPricing: (key, input, output) => set((s) => {
    const next = { ...s.modelPricing, [key]: { input, output } }
    saveJSON('token-tester-model-pricing', next)
    try { (window as any).electronAPI?.writeUserPricing(next).catch(() => {}) } catch {}
    return { modelPricing: next }
  }),
