  ...
  async function runAll() {
    ...
    for (let i = 0; i < updated.length; i++) {
      ...
      try {
        ...
        let result = await window.electronAPI.chatCompletion({
        ...
        if (result.error && /max_tokens.*max_completion_tokens/i.test(result.error)) {
          ...
          result = await window.electronAPI.chatCompletion({
            provider: { type: prov.type, baseUrl: prov.baseUrl, apiKeyEnv: prov.apiKeyEnv, headers: prov.headers },
            model: run.model,
            messages,
            maxTokens: 4096,
        ...
        try {
          ...
          localTokens = await window.electronAPI.countTokens(fullText)
        } catch { /* ignore */ }
                ...
                {queue.map((run: TestRun) => (
                  ...
                  <span className="text-xs text-surface-400 truncate flex-1">
                    ...
                    {(() => {
                      ...
                      return fp ? (
                        <button
                          onClick={() => window.electronAPI.openFile(fp)}
                          className="text-indigo-400 hover:text-indigo-300 underline underline-offset-2 truncate max-w-[200px] inline-block align-bottom"
                          title={fp}
                        >
                          {run.sourceLabel}
                        </button>
                    ...
                    <div className="grid grid-cols-7 gap-4 text-xs">
                      ...
                      <div>
                        ...
                        {displayEntry.filePath ? (
                          <button
                            onClick={() => window.electronAPI.openFile(displayEntry.filePath)}
                            className="text-indigo-400 hover:text-indigo-300 underline underline-offset-2 truncate block max-w-full text-xs text-left"
                            title={displayEntry.filePath}
                          >
                            {displayEntry.file}
                          </button>
                        ) : displayEntry.file ? (
